...
def render_rows(selected: list[dict[str, Any]], language: str) -> str:
    zh = language == "zh"
...
def render_sections(data: dict[str, Any], language: str, categories: set[str]) -> str:
    zh = language == "zh"
    headers = COPY[language]["headers"]
    rendered = []
    for section in data["sections"]:
        ...
        rendered.append(render("section.html", {
            ...
            "REFERENCE_HEADER": escape(headers[2]),
            "SIGNUP_HEADER": escape(headers[3]),
            "ROWS": render_rows(selected, language),
        }))
...
def render_page(data: dict[str, Any], seo: dict[str, Any], language: str) -> str:
    ...
    if zh:
        ...
        language_nav = '<a href="../us/">English</a>'
    else:
        stats = stat(len(selected), copy["total"])
        language_nav = '<a href="../zh/">中文</a>'
    return render("page.html", {
        ...
        "SECTIONS": render_sections(data, language, categories),
...
def build(output: Path) -> None:
    ...
    zh_url = absolute_url(site_url, "zh/")
    en_url = absolute_url(site_url, "us/")
    write(output / "index.html", render("landing.html", {
        "TITLE": escape(f"{seo['title_en']} · {seo['short_title_zh']}"),
        "DESCRIPTION": escape(f"{seo['description_en']} {seo['description_zh']}"),
